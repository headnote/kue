...
Job.prototype.renderUpdate = function () {
    ...
    view.emailTo(this.data.to[0].email);
    view.created_at_meta((new Date(parseInt(this.created_at))).toLocaleString());
...
Job.prototype.renderTimestamp = function (prop) {
    ...
    if (val) {
        view[prop]().text((new Date(parseInt(val))).toLocaleString());
    } else {
